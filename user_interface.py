...
from PyQt5.QtWidgets import (QApplication, QWidget, QGridLayout, QLabel, QPushButton, QLineEdit,
    QCheckBox, QSlider, QVBoxLayout, QHBoxLayout, QSizePolicy, QDoubleSpinBox)
...
class UserInterface(QWidget):
    ...
    def start_motor(self):
        # 这里填你的马达控制逻辑，比如设置PWM为30%或setpoint为20
        if hasattr(self, 'dc_motor_pwm'):
            self.dc_motor_pwm.setValue(30.0)
        elif hasattr(self, 'motor_setpoint'):
            self.motor_setpoint.setValue(20)
        print("Motor started at medium-low speed.")

    def update_diameter_plot(self, x, y, setpoint):
        self.diameter_plot.update_plot(x, y, setpoint)
